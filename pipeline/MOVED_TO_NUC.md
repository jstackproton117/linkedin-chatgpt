...
RedRose NUC (redrose.local), not here. This OneDrive folder is a frozen
backup only — editing files here has no effect on the running app.

Live location: `~/rebel-intel` on RedRose (`ssh redrose`, or `ssh joe@redrose.local`)
...
- Dashboard URL: http://redrose.local:5000
...
Address RedRose by name, not by IP: `redrose.local` (mDNS), or the `redrose`
alias in `~/.ssh/config`. Its current DHCP lease is `10.0.69.160` — the LAN
was renumbered out of `192.168.1.x`, so any hardcoded `192.168.1.233` is dead.
